...
if __name__ == "__main__":
    ...
    for idx, record in run_records.iterrows():
        ...
        if record.type == "flair_tars":
            ...
            wandb.restore(name='best-model.pt', run_path='/'.join(record.run.path), replace=True, root=str(output_dir))
            ...
            if output_dir.exists():
                shutil.rmtree(str(output_dir))
            output_dir.mkdir(parents=True, exist_ok=True)
            wandb.restore(name='model.joblib', run_path='/'.join(record.run.path), replace=True, root=str(output_dir))
            ...
            if output_dir.exists():
                shutil.rmtree(str(output_dir))
            output_dir.mkdir(parents=True, exist_ok=True)
            wandb.restore(name='label_dictionary.json', run_path='/'.join(record.run.path), replace=True, root=str(output_dir))
